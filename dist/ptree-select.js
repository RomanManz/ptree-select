...
context.prototype = {
	...
	fillItems: function() {
		...
		this._items = ' '.repeat(this.offsetY - this.curY < 0 ? 0 : this.offsetY - this.curY).split('').reduce((prev, cur) => { prev.push({ __id: prev.length, name: '', __filler: true, __ignore: true }); return prev; }, []).concat(this.items);
		// The order of the following helper items matter because they are addressed by offset from within some filters!
		if( this.unfiltered ) this._items.splice(this.offsetY, 0, { __id: 'unfiltered', __filler: true, __unfiltered: true, name: this.unfilteredName });
		this._items.splice(this.offsetY, 0, { __id: 'empty', __filler: true, __empty: true, __hidden: true, name: this.emptyName }); // nothing found
		...
		filterSharedMatch: function(noMatch, item, other, otherListName, nameKey, defaultForeignKey, relationships, ctx) {
			...
			*/
			return ( ! noMatch && ( ! relationships || ! relationships[otherListName] || other._items.some(function(oitem) {
				...
				return oitem[relationships[otherListName]] === item[relationships[otherListName]];
			}) ) );
		},
		/**
			...
			for( var i = this.cfg.offsetX, other = null; i < this.curX + this.cfg.offsetX + offset; i++ ) {
				...
				if( ( ! ctx.foreignKeyRelationships || ctx.foreignKeyRelationships[other.name] ) ||
					...
					ret.every(function(item) { // switched from forEach to every to support the fast-exit done in _noMatch mode
						...
						if( ctx.sharedKeyRelationships && ctx.sharedKeyRelationships[other[other._name]] 
